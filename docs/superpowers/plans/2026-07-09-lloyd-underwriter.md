...
- [ ] **Step 3: Start ASP registration immediately** — register Lloyd as an ASP in **A2MCP (pay-per-call)** mode. Service description (use verbatim, aligns with unknown #3 fallback):
  ...
  Submit even though the endpoint isn't live yet (point at the placeholder Vercel URL from Task 1 Step 6) — **the human review is the critical path**; iterate behind it.

- [ ] **Step 4: Verify the four unknowns** — dig through https://web3.okx.com/onchainos/dev-docs (Developer Portal, Payments/APP protocol, Agentic Wallet sections) and the whitepaper https://web3.okx.com/whitepaper/okx-app-whitepaper.pdf. Record in `docs/okx-notes.md` under numbered headings, each with the exact API/command found or "NOT AVAILABLE":
